...
def main():
    print("\n=========================================")
    print("Training YOLOv8 NANO on RAW images...")
    ...
    # handle valid folder naming for YOLO
    ...
    # extract weights and curves
    best_pt = os.path.join(MODEL_DIR, 'run', 'weights', 'best.pt')
    results_png = os.path.join(MODEL_DIR, 'run', 'results.png')
    
    if os.path.exists(best_pt):
        shutil.copy(best_pt, os.path.join(MODEL_DIR, 'best_yolo_nano.pt'))
        print(f"\nAwesome! Best weights saved to {MODEL_DIR}/best_yolo_nano.pt")
