...
MODEL_DIR = "models/yolo_medium_preprocessed"
MAX_EPOCHS = 100
IMG_SIZE = 224
BATCH_SIZE = 32
...
def main():
    print("\n=========================================")
    print("Training YOLOv8 MEDIUM on PREPROCESSED images...")
    ...
    if not os.path.exists(dataset_abs_dir):
        print("Oops, preprocessed dataset not found. Run the preprocessor first!")
        return
    ...
    if os.path.exists(best_pt):
        shutil.copy(best_pt, os.path.join(MODEL_DIR, 'best_yolo_medium_preprocessed.pt'))
        print(f"\nAwesome! Best weights saved to {MODEL_DIR}/best_yolo_medium_preprocessed.pt")
